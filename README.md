...
当前 Popover 为固定 `372 × 790 pt` 的单列决策界面。它使用 AppKit `transient` Popover，打开时不主动激活为普通前台窗口；点击桌面或其他 App 会按菜单栏组件的方式收起。
...
30 日主图展示最近 30 个交易日的收盘价和由同一组真实收盘价计算的 EMA20 趋势线；真实成交使用从数量加权成交价延伸至日期轴的细引导线，以及轴下的交易日／方向／笔数／实际成交总额摘要。交易按 America/New_York 的美股交易日归组，避免北京时间跨日导致同一交易日被错误拆分；曲线上不再重复叠加成交圆点、字母、气泡或浮签。当前没有卖出成交时明确显示为 0，不生成虚假卖点；悬停成交日可同时读取收盘价、加权成交价和成交总额。

界面使用 macOS 系统材质、动态标签色与系统强调色，不强制深色。系统“外观”设为自动时，QQQMBar 会随系统在浅色与深色之间切换。

信息结构围绕一次分档计划展开，并采用一张连续的 macOS 决策纸张，而不是多张互不关联的仪表盘卡片。顶部直接给出金额、倍率、执行日期、规则结论与 `US$200–600` 五档位置；行情段把 30 日收益、成本偏离和 P/E 置于同一报价带，并用可交互价格图呈现收盘价、EMA20、真实 B/S 成交与平均成本；决策段明确显示加码侧采用 `OR`、减码侧采用 `AND`，展示当前触发计数、三项真实输入和精确门槛。三个条件轨道统一按“低值—中性—高值”从左到右排列，并直接把低值区和高值区映射为真实规则中的多投／少投语义，不再使用无数据含义的中央刻度。决策过渡区还会用实时差值说明升至 `US$500` 的三个单项门槛各自还差多少，以及降至 `US$300` 已满足与仍缺少哪些条件；“本期依据”内的整行按钮可展开完整的 `US$600 / 500 / 400 / 300 / 200` 五档规则。账户段以同一成本偏离连接行情和持仓，并用“可用资金 − 本期计划 = 计划后可用”的算式及分段资金轨道说明本期预留和后续覆盖期数。
...
刷新中、联网更新失败、快照校验差异、已确认和完全不可用都有独立状态。联网失败时主界面明确显示“已保留有效快照”，不把旧数据伪装为最新数据；设置页使用同一视觉层级呈现同步来源、校验结果、五档规则和只读安全边界。

界面全部由快照中的真实字段或可复算指标驱动：价格面积图来自 Nasdaq 收盘价，B/S 点来自 IBKR QQQM 成交，决策依据来自 Nasdaq 30 日动量、FRED VIX、CNN 恐惧与贪婪指数及 Invesco P/E，持仓与资金来自已校验的账户快照。联网刷新会用这些数据重算本期建议；CNN 暂不可用时才使用明确标注的 RSI+VIX 备用模型。
